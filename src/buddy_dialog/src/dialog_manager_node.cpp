...
CallbackReturn
DialogManagerNode::on_configure(const rclcpp_lifecycle::State &) {
  ...
  user_input_sub_ = create_subscription<buddy_interfaces::msg::UserInput>(
      ...
      std::bind(&DialogManagerNode::on_user_input, this,
                std::placeholders::_1));
...
CallbackReturn
DialogManagerNode::on_deactivate(const rclcpp_lifecycle::State &) {
...
void DialogManagerNode::on_user_input(
    const buddy_interfaces::msg::UserInput &msg) {
  RCLCPP_INFO(get_logger(), "Dialog input [%s]: %s", msg.session_id.c_str(),
              msg.text.c_str());
}
