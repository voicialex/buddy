...
bool EmotionOnnxModel::load(const std::string& model_dir) {
    ...
    if (!face_detector_.load(haar_path)) {
        RCLCPP_ERROR(logger_, "Failed to load Haar cascade from %s", haar_path.c_str());
        return false;
    }
    ...
    try {
        emotion_session_ = std::make_unique<Ort::Session>(env_, emotion_path.c_str(), session_opts_);
...
ModelResult EmotionOnnxModel::inference(const cv::Mat& frame) {
    ...
    cv::Rect detected_rect(static_cast<int>(fx), static_cast<int>(fy), static_cast<int>(fw), static_cast<int>(fh));
    ...
    int x2 = static_cast<int>(std::min(static_cast<float>(frame.cols), fx + fw + margin_x));
    int y2 = static_cast<int>(std::min(static_cast<float>(frame.rows), fy + fh + margin_y));
...
std::vector<float> EmotionOnnxModel::detect_face(const cv::Mat& bgr_frame) {
    ...
    return {static_cast<float>(faces[best].x),
            static_cast<float>(faces[best].y),
            static_cast<float>(faces[best].width),
            static_cast<float>(faces[best].height)};
}

std::array<float, 7> EmotionOnnxModel::classify_emotion(const cv::Mat& face_crop) {
    ...
    auto output_name_alloc = emotion_session_->GetOutputNameAllocated(0, allocator);
    const char* output_names[] = {output_name_alloc.get()};

    auto output_tensors =
        emotion_session_->Run(Ort::RunOptions{nullptr}, input_names, &input_tensor, 1, output_names, 1);
...
Ort::Value EmotionOnnxModel::mat_to_tensor(const cv::Mat& image, int target_w, int target_h) {
    cv::Mat resized, rgb;
    ...
    for (int c = 0; c < 3; ++c) {
        std::memcpy(tensor_data_.data() + c * plane, channels[c].data, plane * sizeof(float));
    }

    std::array<int64_t, 4> shape = {1, 3, target_h, target_w};
    auto memory_info = Ort::MemoryInfo::CreateCpu(OrtArenaAllocator, OrtMemTypeDefault);
    return Ort::Value::CreateTensor<float>(
        memory_info, tensor_data_.data(), tensor_data_.size(), shape.data(), shape.size());
}
