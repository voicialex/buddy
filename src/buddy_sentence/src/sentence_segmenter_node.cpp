...
CallbackReturn
SentenceSegmenterNode::on_configure(const rclcpp_lifecycle::State &) {
  ...
  cloud_sub_ = create_subscription<buddy_interfaces::msg::CloudChunk>(
      ...
      std::bind(&SentenceSegmenterNode::on_cloud_chunk, this,
                std::placeholders::_1));
  return CallbackReturn::SUCCESS;
}
CallbackReturn
SentenceSegmenterNode::on_activate(const rclcpp_lifecycle::State &) {
  RCLCPP_INFO(get_logger(), "SentenceSegmenterNode: activating");
  return CallbackReturn::SUCCESS;
}
CallbackReturn
SentenceSegmenterNode::on_deactivate(const rclcpp_lifecycle::State &) {
  RCLCPP_INFO(get_logger(), "SentenceSegmenterNode: deactivating");
  return CallbackReturn::SUCCESS;
}
CallbackReturn
SentenceSegmenterNode::on_cleanup(const rclcpp_lifecycle::State &) {
...
CallbackReturn
SentenceSegmenterNode::on_shutdown(const rclcpp_lifecycle::State &) {
  RCLCPP_INFO(get_logger(), "SentenceSegmenterNode: shutting down");
  return CallbackReturn::SUCCESS;
}
CallbackReturn
SentenceSegmenterNode::on_error(const rclcpp_lifecycle::State &) {
  RCLCPP_ERROR(get_logger(), "SentenceSegmenterNode: error");
  return CallbackReturn::SUCCESS;
}

void SentenceSegmenterNode::on_cloud_chunk(
...
SentenceSegmenterNode::segment(const std::string &text) {
