...
def test_save_and_load_session(tmp_store):
    ...
    tmp_store.save_session("sess-001", data)
    loaded = tmp_store.load_session("sess-001")
...
def test_load_nonexistent_session(tmp_store):
    assert tmp_store.load_session("nonexistent") is None


def test_save_and_load_facts(tmp_store):
    facts = [
    ...
    loaded = tmp_store.load_facts()
    assert len(loaded) == 2
    assert loaded[0].content == "用户名叫小明"
    assert loaded[1].category == "preference"


def test_load_empty_facts(tmp_store):
    assert tmp_store.load_facts() == []


def test_load_knowledge(tmp_store):
    knowledge_file = tmp_store.data_dir / "knowledge.yaml"
    knowledge_file.write_text("主人叫张三，家里有一只猫叫咪咪。\n", encoding="utf-8")
    result = tmp_store.load_knowledge()
    assert "张三" in result
    assert "咪咪" in result


def test_load_knowledge_missing(tmp_store):
    assert tmp_store.load_knowledge() == ""
...
def test_list_sessions(tmp_store):
    for sid in ["sess-001", "sess-002"]:
        ...
        tmp_store.save_session(sid, data)
